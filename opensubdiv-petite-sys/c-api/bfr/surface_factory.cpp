...
Bfr_SurfaceFactory_f *Bfr_SurfaceFactory_Create(
    ...
    int approx_level_smooth,
    int approx_level_sharp) {
    if (!refiner) {
        return nullptr;
    }
...
Bfr_Surface_f *Bfr_Surface_Create() { return new Bfr_Surface_f(); }

void Bfr_Surface_Destroy(Bfr_Surface_f *surface) { delete surface; }

bool Bfr_SurfaceFactory_InitVertexSurface(
    const Bfr_SurfaceFactory_f *factory,
    int face_index,
    Bfr_Surface_f *surface) {
...
bool Bfr_Surface_GatherPatchPoints(
    ...
    std::memcpy(out_patch_points,
                patch_points.data(),
                patch_points.size() * sizeof(float));
    return true;
}
