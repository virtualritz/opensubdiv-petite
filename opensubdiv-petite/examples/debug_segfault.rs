...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Patch table created with {} patches", patch_table.patches_len());
    ...
    for level in 1..refiner.refinement_levels() {
        ...
        println!("Level {}: interpolating from {} vertices", level, prev_count);
    ...
    if num_local_points > 0 {
        if let Some(stencil_table) = patch_table.local_point_stencil_table() {
            ...
            println!("  Control vertex count: {}", stencil_table.control_vertex_count());
            ...
            if stencil_table.control_vertex_count() != all_vertices.len() {
                println!("ERROR: Stencil table expects {} control vertices but we have {}",
                         stencil_table.control_vertex_count(), all_vertices.len());
