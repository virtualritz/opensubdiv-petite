...
#[test]
fn test_metal_stencil_evaluation() {
    ...
    let output_path = test_output_path("metal_stencil_evaluation.txt");
    let mut file = File::create(&output_path).expect("Failed to create output file");

    writeln!(file, "Metal Stencil Evaluation Test Results").unwrap();
    writeln!(file, "=====================================").unwrap();
    writeln!(file).unwrap();
    ...
    assert_file_matches(&output_path, "metal_stencil_evaluation.txt");
}

#[cfg(not(all(feature = "metal", target_os = "macos")))]
#[test]
fn test_metal_stencil_evaluation() {
    println!("Metal test skipped: feature 'metal' not enabled or not on macOS");
}
