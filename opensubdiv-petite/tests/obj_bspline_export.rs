...
fn test_simple_plane_bspline_obj() {
    ...
    patch_table.export_obj_bspline_file(output_path.to_str().unwrap(), &all_vertices)
...
fn test_two_patches_bspline_obj() {
    ...
    use std::io::Write;
    writeln!(file, "# OpenSubdiv B-spline Surface Export - First Two Patches Only").unwrap();
            ...
            if let Some(cv_indices) = patch_table.patch_array_vertices(array_idx) {
                ...
                for patch_idx in 0..patch_table.patch_array_patches_len(array_idx) {
                    ...
                    writeln!(file, "# Patch {} (array {}, local {})", 
                        patch_count, array_idx, patch_idx).unwrap();
